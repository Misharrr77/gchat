...
export default function ChatPage() {
  ...
  return (
    <>
      ...
      {/* Drawer overlay - OUTSIDE flex container to avoid overflow clipping */}
      {drawer && (
        <div className="fixed inset-0 z-[100]" onClick={() => setDrawer(false)}>
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[280px] bg-dark-900 shadow-2xl drawer-panel flex flex-col border-r border-dark-600" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 py-1">
              <DrawerBtn icon={<UserPlus size={18} />} label="Новый чат" onClick={() => setDrawer(false)} />
              <DrawerBtn icon={<Users size={18} />} label="Создать группу" onClick={() => setDrawer(false)} />
              <DrawerBtn icon={<Radio size={18} />} label="Создать канал" onClick={() => setDrawer(false)} />
              <DrawerBtn icon={<Compass size={18} />} label="Найти" onClick={() => setDrawer(false)} />
            </div>
            <div className="border-t border-dark-600 p-2">
              <button onClick={() => { setDrawer(false); logout(); }} className="w-full flex items-center gap-3 px-5 py-3 text-red-400 hover:bg-dark-700 rounded-xl transition text-sm">
