...
import { Send, Image, Music, X, Loader } from 'lucide-react';
...
export default function MessageInput() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (preview) {
        ...
        await sendMessage(text.trim() || '', preview.type === 'audio' ? 'audio' : 'image', d.url);
        setPreview(null);
      } else {
  ...
  const pickFile = (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'audio') => {
    const f = e.target.files?.[0];
    if (!f) return;
    setPreview({ type, url: URL.createObjectURL(f), file: f });
    e.target.value = '';
  };
