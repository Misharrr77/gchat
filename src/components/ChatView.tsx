...
interface Props { onBack: () => void; onProfile: (u: User) => void; }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 py-3 bg-dark-800 border-b border-dark-600 flex-shrink-0">
        <button onClick={onBack} className="md:hidden p-2 hover:bg-dark-700 rounded-lg text-slate-400 flex-shrink-0">
          <ArrowLeft size={20} />
        </button>
        ...
        {(isGroup || isChannel) && (
          <button onClick={() => setShowGroupProfile(true)} className="p-2 hover:bg-dark-700 rounded-lg text-slate-400 flex-shrink-0">
            <Info size={18} />
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {loadingMsgs ? (
          ...
          <div className="flex items-center justify-center h-full"><p className="text-slate-500 text-sm">{isChannel ? 'Нет публикаций' : 'Нет сообщений. Напишите первым!'}</p></div>
        ) : messages.map((msg, i) => (
      ...
      {canWrite ? <MessageInput /> : (
        <div className="border-t border-dark-600 bg-dark-800 px-4 py-3 text-center text-sm text-slate-500">Только админы могут писать в канал</div>
      )}
